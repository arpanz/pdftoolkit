...
class _PdfToImagesScreenState extends State<PdfToImagesScreen> {
  ...
  Future<void> _convert() async {
    ...
    try {
      ...
      final outDir = p.join(
        dir.path, 'BatchPDF', 'images_${DateTime.now().millisecondsSinceEpoch}',
      );
      ...
      if (result.success) {
        final imagePaths = result.outputPath.split('|').where((s) => s.isNotEmpty).toList();
        if (mounted) {
          Navigator.push(
            context,
            MaterialPageRoute(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                _DpiChip(label: '72 DPI', sublabel: 'Screen', value: 72,
                    selected: _dpi == 72, onTap: () => setState(() => _dpi = 72)),
                const SizedBox(width: 8),
                _DpiChip(label: '150 DPI', sublabel: 'Balanced', value: 150,
                    selected: _dpi == 150, onTap: () => setState(() => _dpi = 150)),
                const SizedBox(width: 8),
                _DpiChip(label: '300 DPI', sublabel: 'Print', value: 300,
                    selected: _dpi == 300, onTap: () => setState(() => _dpi = 300)),
              ],
  ...
  Future<void> _saveAsZip(BuildContext context) async {
    try {
      ...
      final zipPath = p.join(
          dir.path, 'BatchPDF', 'pages_${DateTime.now().millisecondsSinceEpoch}.zip');
